  ...
  return (
    <View
      style={{
        ...
        padding: 18,
        borderWidth: 1,
        borderColor: done ? t.lime : isInProgress ? t.orange : t.line2,
        shadowColor: "#000",
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 9 }}>
        ...
        <Text
          style={{
            fontSize: 12.5,
            fontWeight: "800",
            color: done ? t.limeInk : isInProgress ? t.orange : t.mut,
            letterSpacing: 0.3,
          }}
        >
          {done
        ...
        {!done && !isInProgress && onSwap && (
          ...
          >
            ...
            <Text style={{ fontWeight: "800", fontSize: 12.5, color: t.text }}>
              Swap
            </Text>
          </Pressable>
        )}
      ...
      >
        {shown.map((e: string) => (
          <View
            key={e}
            style={{
              backgroundColor: done ? t.limeSoft : t.elev,
              paddingVertical: 7,
              paddingHorizontal: 11,
              borderRadius: 10,
            }}
      ...
      <View style={{ marginTop: 18 }}>
        {done ? (
          ...
          >
            ...
            <Text
              style={{ fontWeight: "800", fontSize: 15.5, color: t.limeInk }}
            >
              Done
            </Text>
          </View>
          ...
          <Btn
            ...
            onPress={onResume}
            icon="play"
            style={{ borderRadius: 16, backgroundColor: t.orange }}
          >
            Resume workout
          </Btn>
        ) : (
    ...
    <View style={{ flexDirection: "row", gap: 9 }}>
      ...
      >
        ...
        <View style={{ flexDirection: "row", alignItems: "baseline", gap: 3 }}>
          ...
          />
          <Text style={{ fontSize: 11, color: t.mut, fontWeight: "700" }}>
            {fmt.wlabel}
          </Text>
        </View>
      </View>
      ...
      >
        ...
        <View style={{ flexDirection: "row", alignItems: "baseline", gap: 3 }}>
          <TextInput
            value={bf}
            onChangeText={(v) => setBf(v.replace(/[^0-9.]/g, ""))}
            placeholder="—"
            placeholderTextColor={t.mut2}
            keyboardType="decimal-pad"
            style={{
      ...
      <Pressable
        onPress={onSave}
        style={{
          backgroundColor: w ? t.lime : t.elev,
...
function MeasureCard() {
  ...
  const loggedToday =
    lastMeasurement &&
    Date.now() - lastMeasurement.at < DAY_MS &&
    new Date(lastMeasurement.at).getDate() === new Date().getDate();
  ...
  const openEdit = () => {
    setW(lastMeasurement ? String(fmt.w(lastMeasurement.weightKg)) : "");
    setBf(
      lastMeasurement?.bodyFat != null ? String(lastMeasurement.bodyFat) : "",
    );
    setEditing(true);
  };
  ...
  if (editing) {
    return (
      ...
      >
        ...
        >
          <Icon name="ruler" size={16} color={t.mut} sw={2} />
          <Text
            style={{
              flex: 1,
              fontSize: 14.5,
          ...
          <Pressable onPress={() => setEditing(false)}>
            <Text style={{ color: t.mut, fontWeight: "700", fontSize: 14 }}>
              Cancel
            </Text>
          </Pressable>
        </View>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          {lastMeasurement && (
            ...
            >
              {fmt.w(lastMeasurement.weightKg)} {fmt.wlabel}
              {lastMeasurement.bodyFat
                ? ` · ${lastMeasurement.bodyFat}% body fat`
                : ""}
            </Text>
          )}
        </View>
        <Pressable onPress={openEdit}>
          <Text style={{ color: t.orange, fontWeight: "800", fontSize: 14 }}>
            Edit
          </Text>
        </Pressable>
      </View>
...
export function HomeScreen() {
  ...
  const ov =
    state.override && state.override.day === today
      ? state.routines.find((r) => r.id === state.override!.id)
      : null;
  const todays = ov ? [ov] : sched;
  const swappedFrom = ov
    ? sched.length
      ? sched.map((r) => r.name).join(" / ")
      : "a rest day"
    : null;
  ...
  const sessionForRoutine = (routineId: string) =>
    recentHistory.find(
      (s) => s.routineId === routineId && s.startedAt >= todayStart.getTime(),
    ) ?? null;

  // True when there's an in-progress session not matched by today's scheduled routines
  const orphanResume =
    inProgressSession &&
    !todays.some((r) => r.id === inProgressSession.routineId);
  ...
  return (
    ...
    >
      <ScrollView
        ...
        contentContainerStyle={{ padding: 18, paddingBottom: 24, gap: 18 }}
      ...
      >
        ...
        >
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 14, color: t.mut, fontWeight: "700" }}>
              {greeting},
            </Text>
            <Text
              style={{
          ...
          >
            ...
            <Text style={{ fontSize: 12, color: t.mut, fontWeight: "700" }}>
              this wk
            </Text>
          </View>
        </View>
        ...
        {orphanResume && (
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text
                style={{ fontSize: 14.5, fontWeight: "800", color: t.text }}
              >
                Workout in progress
              </Text>
              <Text
              ...
              >
                {inProgressSession!.routineName} · tap to resume
              </Text>
            </View>
        ...
        {todays.length > 0 ? (
          <View style={{ gap: 12 }}>
            {todays.map((r) => (
              <RoutineCard
                ...
                todaySession={sessionForRoutine(r.id)}
                isInProgress={inProgressSession?.routineId === r.id}
          ...
          >
            ...
            >
              No routine scheduled today. Recovery is part of the plan — or jump
              in anyway.
            </Text>
            <View
              style={{
          ...
          <View>
            ...
            >
              ...
              <Pressable onPress={() => nav("history")}>
                <Text
                  style={{ color: t.orange, fontWeight: "800", fontSize: 13.5 }}
                >
                  See all
                </Text>
              </Pressable>
            ...
            >
              {recent.map((s) => (
                ...
                >
                  <Text
                    style={{ fontSize: 12, color: t.mut, fontWeight: "700" }}
                  >
                    {relDate(s.startedAt)}
                  </Text>
                  <Text
                  ...
                  >
                    <View>
                      ...
                      >
                        {sessionVolume(s.exercises).toLocaleString()}
                      </Text>
                      <Text
                        style={{
                          fontSize: 10.5,
        ...
        >
          ...
          >
            Do a different routine today — your weekly schedule won't change.
          </Text>
          <View style={{ gap: 9 }}>
            {ov && (
              <Pressable
              ...
              return (
                <Pressable
                  ...
                  style={{
                    ...
                    padding: 14,
                    borderRadius: 16,
                    backgroundColor: active ? t.limeSoft : t.surface2,
